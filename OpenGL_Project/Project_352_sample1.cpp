...
} carObject = { -100, -100, 10, 1, 0, 0, 0 }, //where make movements here
CarReset = { initial_W / 2 + 50, initial_H / 2 + 50, 10, 6, 0, 0, 0 }; //a copy of the original coordinates
...
int main(int argc, char** argv) {
	...
	glutAddMenuEntry("Camera gets farther from the house", 1);
	glutAddMenuEntry("Camera gets closer to the house", 2);
	glutAddMenuEntry("Camera viewing the whole scene from above the street", 3);
	...
	//gluLookAt(0, 10, 10, 0, -30000, 0, 0, 0, 1);
...
void DrawXRoad()
{
	...
	glVertex3f(1.1, 0.1, -0.2);
	glVertex3f(1.1, 0.1, -0.7);
	glVertex3f(-1, 0.1, -0.7);
	glEnd();
	...
	for (int i = 0; i < 10; i++)
	{
		...
		glBegin(GL_POLYGON);
		glVertex3f(-1 + m * i, 0.1, -0.4);
		glVertex3f(-0.8 + m * i, 0.1, -0.4);
		glVertex3f(-0.8 + m * i, 0.1, -0.5);
		glVertex3f(-1 + m * i, 0.1, -0.5);
		glEnd();
...
void DrawYRoad()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		glBegin(GL_POLYGON);
		glVertex3f(-0.2, 0.1, -1.5 + m * i);
		glVertex3f(-0.3, 0.1, -1.5 + m * i);
		glVertex3f(-0.3, 0.1, -1.3 + m * i);
		glVertex3f(-0.2, 0.1, -1.3 + m * i);
		glEnd();
...
void drawFrame() {
	...
	glTranslated(carObject.x, carObject.y, 0); //translate the square
